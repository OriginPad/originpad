...
3. **Bond.** The 100th mint triggers the token factory. An ERC-20 (1B supply)
   ...
   pool and locked.
4. **Trade.** The built-in NFT marketplace unlocks: browse floor-first, buy
   listed NFTs, list your own with an on-chain expiry, or place one
   collection-wide offer held in escrow that any holder can accept (cancel
   anytime for a refund). The token trades on the V4 pool through an in-app
   swap. Everything settles inside the contracts, with no external approvals.
